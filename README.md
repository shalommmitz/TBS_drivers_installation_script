...
For reboot persistence, the installer also writes `/etc/modprobe.d/tbs-dvb.conf` soft dependencies when a PCI bridge needs explicit frontend helpers. For TBS 6909 this writes `softdep tbsecp3 pre: mxl58x`, so a PCI auto-probe of `tbsecp3` still loads `mxl58x` first. After writing that file, the installer refreshes the running kernel's initramfs so the same soft dependency is available during early boot coldplug, not only after the root filesystem is mounted.
